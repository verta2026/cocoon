...
> 域名+vps，手机随时随地打开 `chat.xxx.top` 就能和你的 AI 聊天。
...
> 因为 Cloudflare 代理了流量并提供 HTTPS，nginx 这边只监听 80 端口也能跑——但此时 Cloudflare 的 SSL 模式要选 **Flexible**（选 Full 会 522：Full 要求你的 VPS 在 443 上有证书）。想让 Cloudflare → VPS 这一段也加密（推荐），在 Cloudflare 签一张 Origin 证书装进 nginx、监听 443，再把 SSL 模式切到 **Full (strict)**。
>
> 反代就位后，在 cocoon 的 `.env` 里加 `COCOON_TRUST_FORWARDED_PROTO=1`——登录 cookie 会带上 `Secure` 标记，只在 HTTPS 上传输。
...
- 用 Caddy：选 **Full (strict)**（Caddy 自动签真证书）
- 用 nginx 且装了 Cloudflare Origin 证书（监听 443）：选 **Full (strict)**
- 用 nginx 只监听 80（没证书）：选 **Flexible**——cocoon 不做 http→https 跳转，不会出现重定向循环；代价是 Cloudflare 到 VPS 这段是明文，能升级就升级

## 省钱小贴士

- `.top` 域名首年通常 1-8 元，次年不建议续费，建议开个新号继续买特价域名
