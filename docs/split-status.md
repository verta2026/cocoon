...
- `bridge.output_routes` - terminal output, raw output, optional messages, and
  optional incremental pure-chat stream (/chat_pure)
- `bridge.live_archive` - session-jsonl chat mirror provider for /messages,
  with external-send and external-recv sidecar merge, double-record dedup,
  cross-source dedup, and an incremental pure-chat stream with stable
  time-monotonic ids for structured chat UIs
...
- `bridge.forge_window` provides raw/dialogue/dropped zone construction,
