...
**Outbound messages don't reach the web chat log (by default).** Replies the agent sends via Telegram go through the plugin's outbound path, not the Claude session file. To mirror them into the web UI, apply the sidecar patch to the plugin (`patches/telegram-sidecar.patch`, see `patches/README.md` — a small Apache-2.0-compatible diff that makes the plugin append sends/receives to jsonl sidecars, enabled only when `TELEGRAM_SIDECAR_DIR` is set), then point `COCOON_SEND_SIDECAR_FILE` at `$TELEGRAM_SIDECAR_DIR/_telegram_sends.jsonl`. Inbound needs nothing — Telegram messages enter the session as `<channel>` tags, which the web UI renders natively; set `COCOON_PRIMARY_SENDER_ID` to render your own Telegram account as "you" rather than a third-party bubble.
