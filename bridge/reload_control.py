...
def set_auto_reload_paused(
    pause_file: Path, log_file: Path, paused: bool, *, force: bool = False
) -> dict:
    ...
    if paused:
        ...
        # A pause the user set by hand must not be lifted by UI toggles or
        # status syncs that happen to post paused=false; clearing a
        # "manual-pause"-marked file requires an explicit force=true.
        content = ""
        try:
            content = pause_file.read_text(encoding="utf-8")
        except OSError:
            pass
        if "manual-pause" in content and not force:
            log_auto_reload(log_file, "unpause blocked: manual-pause requires force=true")
            return auto_reload_status(pause_file)
        try:
            pause_file.unlink()
        except FileNotFoundError:
            pass
        log_auto_reload(log_file, "manual pause disabled" + (" (forced)" if force else ""))
    return auto_reload_status(pause_file)
