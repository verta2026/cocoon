...
def _user_marker_hit(text: str, markers, max_len: int = _BARE_MARKER_MAX_LEN) -> bool:
    ...
    for marker in markers:
        ...
        if marker.startswith("<"):
            if marker in text:
                return True
        elif first_line.startswith(marker) or (len(text) <= max_len and marker in first_line):
            # A handoff block *begins* with its marker → filter regardless of
            # length; a long message that merely quotes the marker mid-line is
            # still spared by the length cap.
            return True
    return False
