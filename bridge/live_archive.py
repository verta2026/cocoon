...
def bare_marker_noise(text: str, markers, max_len: int = BARE_MARKER_MAX_LEN) -> bool:
    ...
    # A handoff/plumbing block *begins* with its marker → filter regardless of
    # length. A long real message that merely quotes the marker mid-line is only
    # matched under the length cap, so it survives.
    if any(first_line.startswith(marker) for marker in markers):
        return True
    if len(text) > max_len:
        return False
    ...
    for row in rows:
        ...
        if src == transcript_source and row.get("role") == "assistant":
            ...
            if cid and mid and (cid, mid) in chan_ids:
                continue
            ts = timestamp_epoch(row)
            # Recv sidecar rows may arrive without a chat_id; they can still be
            # duplicates of an identity-carrying <channel> row, so fall back to
            # every bucket with the same content instead of missing them all.
            if cid:
