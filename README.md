...
Cocoon is a starting point, not a finished product. The codebase is compact — one FastAPI server plus a single-file frontend — and meant to be forked and modified. Swap the theme, add features, connect it to Telegram or Discord, change whatever you want.
...
| `COCOON_AUTO_RELOAD_ENABLED` | `0` | Starts the built-in auto-reload monitor. Requires `COCOON_RELOAD_COMMAND`; when real context usage (read from the session jsonl) crosses the threshold and Claude is idle, the reload command is sent automatically |
...
| `COCOON_MAX_UPLOAD_MB` | `200` | Upload size cap in MB. `0` removes the limit |
...
| `COCOON_AUTO_ACCEPT_SETTINGS_WARNING` | `0` | Also auto-accept the Claude Code settings warning. Off by default — that warning usually means a config file is broken and deserves a look |
...
- **Auto-dismiss prompts** — handles resume, rating and folder-trust prompts automatically (the settings warning stays manual unless explicitly enabled)
...
> There's a whole guide written for the agent itself — markup, stickers,
> popups, manners: [docs/for-your-agent.md](docs/for-your-agent.md).
> Point your `CLAUDE.md` at it and your agent learns the frontend in one read.

Stickers are a two-way feature: you send them from the panel, and **the agent
can send them back** — but the agent never sees the images. It reads text.

**In the chat page** (structured frontend): the sticker button in the composer
opens the panel. `＋` uploads an image (resized to 256px, PNG). Tap a sticker
to send it. **Long-press a sticker to edit it** — rename it, edit its
description, or delete it. The description matters more than it looks: it's
what the agent reads instead of the image.

**On the wire**: a sticker message is just text — `[sticker:<filename>|<name>|<desc>]`
(name/desc embedded from `meta.json` so the agent reads what the sticker means;
a bare `[sticker:<filename>]` also works).
The frontend renders that markup as the image from `/stickers/<filename>`;
everything else (history, bridge, Claude's transcript) sees plain text.

**What the agent sees**: `meta.json` in the sticker directory
(`COCOON_STICKER_DIR`, default `<tmp>/cocoon-stickers`) maps each filename to
its name and description:

```json
{ "happy_cat.png": { "name": "happy cat", "desc": "smug satisfaction, use after shipping something" } }
```

To let your agent use stickers, point it at that file from your `CLAUDE.md`:

```markdown
Stickers: to send one in chat, write [sticker:<filename>] in your reply.
Pick the right one by reading <sticker dir>/meta.json first — the desc
field tells you what each sticker means. Never invent filenames.
```

Write descriptions for a reader who can't see the picture — mood plus
occasion ("exhausted but victorious, use after a long debugging night")
beats a visual description. Editing a sticker's name or description in the
panel updates `meta.json`, so the agent picks it up on its next look.
...
├── requirements.txt
├── frontend/         # static assets (login, editor, config, avatars)
├── webapp/           # the chat frontend (React/Vite, served at /app/)
└── bridge/
...
cd webapp
npm install
npm run build     # outputs webapp/dist (static files)
...
Cocoon auto-dismisses this prompt. If it still hangs, the prompt may have appeared before cocoon's status check. Hit `/status` to trigger the dismiss — pass the token in the `Authorization` header, never in the URL (URL tokens leak into logs and history):

```
curl -H "Authorization: Bearer your-token" http://localhost:8080/status
```

Or restart with `POST /start`.
...
```
git pull
./start.sh          # re-checks and installs any new dependencies
```

If `webapp/` changed in the pull, rebuild the React frontend once:

```
cd webapp && npm install && npm run build
```

Your own files are never touched by an update: `config.js`,
