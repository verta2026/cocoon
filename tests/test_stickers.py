...
class StickersTest(unittest.TestCase):
    def test_upload_list_edit_and_delete_sticker(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            png_bytes = b"\x89PNG\r\n\x1a\n" + b"x" * 8
            uploaded = upload_sticker_file(sticker_dir, meta_path, FakeUpload("../wave.png", png_bytes), "Wave", "hello")
            self.assertEqual(uploaded, {"file": "wave.png", "name": "Wave"})
            self.assertEqual((sticker_dir / "wave.png").read_bytes(), png_bytes)

            # 非图片字节头直接 400——贴纸目录不做任意文件投放点
            with self.assertRaises(HTTPException) as ctx:
                upload_sticker_file(sticker_dir, meta_path, FakeUpload("evil.png", b"#!/bin/sh rm -rf"), "x", "")
            self.assertEqual(ctx.exception.status_code, 400)

            # 超过上限 413，且残留文件被清掉
            with self.assertRaises(HTTPException) as ctx:
                upload_sticker_file(sticker_dir, meta_path, FakeUpload("big.png", png_bytes + b"y" * 64), "x", "", max_bytes=16)
            self.assertEqual(ctx.exception.status_code, 413)
            self.assertFalse((sticker_dir / "big.png").exists())
...
class StickerDataUploadTest(unittest.TestCase):
    ...
    def test_data_url_upload_saves_file_and_meta(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            meta = load_sticker_meta(meta_path)["my_dot.png"]
            self.assertEqual(meta["name"], "dot")
            # 添加即写描述：AI 只认识 meta 里的文字，不认识图片本身
            self.assertEqual(meta["desc"], "a tiny dot for tests")
